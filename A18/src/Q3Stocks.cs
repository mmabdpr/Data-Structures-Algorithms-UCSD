...
namespace A8
{
    public class Q3Stocks : Processor
    {
        ...
        public virtual long Solve(long stockCount, long pointCount, long[][] info)
        {
        
            // graph = initial residual graph
            var vertexCount = (int) stockCount * 2 + 2;
            ...
            for (var i = 0; i < stockCount; ++i) {
                for (var j = 0; j < stockCount; ++j) {
                    ...
                    var stockBelowStock = true;
                    for (var p = 0; p < pointCount; p++) {
                        if (info[i][p] < info[j][p]) continue;
                        stockBelowStock = false;
                        break;
                    }
            ...
            for (var i = 0; i < stockCount; i++) {
                ...
                graph.AddEdge((int)stockCount + i + 1, vertexCount - 1, 1);
            }

            var maxflow = FordFulkerson(graph);
            var ans = stockCount - maxflow;
            return ans;
        }
